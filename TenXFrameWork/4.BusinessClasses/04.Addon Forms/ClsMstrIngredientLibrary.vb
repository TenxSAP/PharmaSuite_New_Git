Public Class ClsMstrIngredientLibrary
...
#End Region
    ...
    Sub CreateForm()
        ...
        Try
            ...
            objMain.objUtilities.LoadForm("MstrIngredientLibrary.xml", "frm_INGLIB", ResourceType.Embeded)
            objForm = objMain.objApplication.Forms.GetForm("frm_INGLIB", objMain.objApplication.Forms.ActiveForm.TypeCount)
            objForm.Freeze(True)
            objutilities = New Utilities
            oDBs_Head = objForm.DataSources.DBDataSources.Item("@TNX_ING_LIB")
            ...
            rs.DoQuery("SELECT TOP 1 ""Code"" FROM ""@TNX_ING_LIB""")
    ...
    Sub MenuEvent(ByRef pVal As SAPbouiCOM.MenuEvent, ByRef BubbleEvent As Boolean)
        ...
        Try
            ...
            If pVal.MenuUID = "10X_RD_INGLIB" And pVal.BeforeAction = False Then
                Me.CreateForm()
    ...
    Sub SetNewLine(ByVal FormUID As String)
        Try
            ...
            oDBs_Head.SetValue("U_ItemCode", rowIndex, "")
            oDBs_Head.SetValue("U_IngredientType", rowIndex, "")
            oDBs_Head.SetValue("U_CASNo", rowIndex, "")
            oDBs_Head.SetValue("U_PharmaGrade", rowIndex, "Y")
            oDBs_Head.SetValue("U_Manufacturer", rowIndex, "Y")
            oDBs_Head.SetValue("U_DefaultUOM", rowIndex, "")
            oDBs_Head.SetValue("U_StorageCondition", rowIndex, "")
            oDBs_Head.SetValue("U_ShelfLifeDays", rowIndex, "")
            oDBs_Head.SetValue("U_QCRequired", rowIndex, "Y")
    ...
    Public Sub MatrixLoad()
        Try
            ...
            Dim rs1 As String
            rs1 = "SELECT ""Code"",""Name"", ""U_ItemCode"", ""U_IngredientType"", ""U_CASNo"", ""U_PharmaGrade"", ""U_Manufacturer"", ""U_DefaultUOM"", ""U_StorageCondition"", ""U_ShelfLifeDays"", ""U_QCRequired"", ""U_Status"" " & "FROM ""@TNX_ING_LIB"" ORDER BY ""Code"""
            ...
            If ors1.RecordCount > 0 Then
                For j As Integer = 0 To ors1.RecordCount - 1
                    ...
                    oDBs_Head.SetValue("U_ItemCode", j, ors1.Fields.Item("U_ItemCode").Value.ToString())
                    oDBs_Head.SetValue("U_IngredientType", j, ors1.Fields.Item("U_IngredientType").Value.ToString())
                    oDBs_Head.SetValue("U_CASNo", j, ors1.Fields.Item("U_CASNo").Value.ToString())
                    oDBs_Head.SetValue("U_PharmaGrade", j, ors1.Fields.Item("U_PharmaGrade").Value.ToString())
                    oDBs_Head.SetValue("U_Manufacturer", j, ors1.Fields.Item("U_Manufacturer").Value.ToString())
                    oDBs_Head.SetValue("U_DefaultUOM", j, ors1.Fields.Item("U_DefaultUOM").Value.ToString())
                    oDBs_Head.SetValue("U_StorageCondition", j, ors1.Fields.Item("U_StorageCondition").Value.ToString())
                    oDBs_Head.SetValue("U_ShelfLifeDays", j, ors1.Fields.Item("U_ShelfLifeDays").Value.ToString())
                    oDBs_Head.SetValue("U_QCRequired", j, ors1.Fields.Item("U_QCRequired").Value.ToString())
